...
public class JobData {
    ...
    public static ArrayList<HashMap<String, String>> findByValue(String searchTerm) {
        ...
        // Step 1: Loop through allJobs
        for (Integer i = 0; i < allJobs.size(); i++) {
            HashMap<String, String> row = allJobs.get(i);

            // Step 2: loop over job using forEach
            for (Map.Entry<String, String> column : row.entrySet()) {
                // Step 3: compare searchTerm against column data
                String columnValue = column.getValue().toLowerCase();
                if (columnValue.contains(searchTerm)) {
                    // Step 4: add to matchingJobs
                    matchingJobs.add(row);
                    //step 4.1: break when 1 instance is found
                    break;
                }
